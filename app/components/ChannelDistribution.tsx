...
const COLORS = ['#38bdf8', '#a78bfa', '#34d399', '#fbbf24', '#f472b6', '#22d3ee', '#fb7185', '#94a3b8'];
              ...
              content={({ active, payload }) => {
                ...
                return (
                  <div className="rounded-lg border border-slate-700 bg-slate-900/95 px-3 py-2 text-xs shadow-xl">
                    <div className="font-medium text-slate-100">{p.source}</div>
                    <div className="text-slate-400">{formatBRL(p.faturamento)}</div>
                    <div className="text-slate-400">{formatPercent(p.share)}</div>
                  </div>
                );
              }}
            />
